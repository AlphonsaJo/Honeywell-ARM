...
// Ensure the correct compiler flags for ARM NEON support
// For example: g++ test.cpp -o test -mfloat-abi=softfp -mfpu=neon

#ifdef __ARM_NEON
#include <arm_neon.h> // Include ARM NEON header only if compiling for ARM with NEON support
#else
...
static void vsubw_arm_neon(int16_t* vecA, int16_t* vecB, int32_t* result, size_t size) {
    for (size_t i = 0; i < size; i += 4) {
        ...
        int32x4_t res = vsubl_s16(a, b); // Correct usage of widening subtraction
